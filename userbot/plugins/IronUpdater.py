...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit('`**Ironbot** Berhasil Di Update!`')
...
async def upstream(event):
    ...
    if conf is None and force_update is False:
        changelog_str = f'**UPDATE Terbaru Untuk Ironbot [{ac_br}]:\n\nPERUBAHAN:**\n`{changelog}`'
    ...
    if force_update:
        ...
        await event.edit('`Proses Update Ironbot,.🛠️`')
        await event.edit('`Proses Update Ironbot,..🛠️`')
        await event.edit('`Proses Update Ironbot,...🛠️`')
        await event.edit('`Proses Update Ironbot,....🛠️`')
    if conf == "now":
